...
def extract_features():
    ...
    stk_tickers = ['JPM', 'GS', 'C']
    ccy_tickers = ['DEXCAUS', 'DEXUSUK']
    idx_tickers = ['SP500', 'DJIA', 'WILL5000PR']
    ...
    Y = np.log(stk_data.loc[:, ('Adj Close', 'GS')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1]+'_Future'
    
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('JPM', 'C'))]).diff(return_period)
